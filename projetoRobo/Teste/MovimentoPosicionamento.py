...
def zonaLimite():
	...
	if (eixoXF < 0) or (eixoYF < 0) or (eixoXF > 20) or (eixoYF > 20):
		return False
	else:
		setEixoX(eixoXF)
		setEixoY(eixoYF)
		setOrientacao(orientacaoF)
		return True

def paraString():
	print(eixoX, eixoY, orientacao)
...
def move():
	...
	if direcao == '0':
		...
		elif getOrientacao() == 4:
			setOrientacao(2)
			
		else:
			setOrientacao(getOrientacao()+2)

		pass

	if getOrientacao() == 1:	 # Norte
		setEixoY(getEixoY()+1)
		
	elif getOrientacao() == 2:	 # Leste
		setEixoX(getEixoX()+1)
		
	elif getOrientacao() == 3:	 # Sul
		setEixoY(getEixoY()-1)
		
	elif getOrientacao() == 4:	 # Oeste
		setEixoX(getEixoX()-1)
